...
// MOS8520 CIA — modified 6526 with simplified timers and interrupts for IEC drive.
...
// Internal register file: ports, direction, timers, serial, interrupts, control.
reg [7:0] pra, prb, ddra, ddrb;
reg [7:0] ta_lo, ta_hi, tb_lo, tb_hi;
reg [7:0] sdr;
reg [4:0] imr, icr;
reg       timer_b_int;
reg [7:0] cra, crb;
reg [15:0] timer_a, timer_b;
reg [23:0] tod_count, tod_alarm, tod_latch;
reg        tod_prev, tod_run, tod_alarm_reg, tod_latched;
reg        sp_pending, sp_transmit;
reg [7:0]  sp_shiftreg;
reg        icr3;
reg        cnt_in_prev;
reg        cnt_out_r;
...
// Read and write strobes derived from phi2 and chip select.
...
// Port A output with direction masking.
always @(posedge clk) begin
	if (!res_n) begin
		pra  <= 8'h00;
		ddra <= 8'h00;
...
// Port B output with timer-toggle bits on PB7/PB6.
always @(posedge clk) begin
	if (!res_n) begin
		prb  <= 8'h00;
		ddrb <= 8'h00;
...
// FLAG input edge detection and ICR[4] flag.
always @(posedge clk) begin
	reg old_flag, flag;
...
// Port C handshake output pulse generation.
reg pcr;
always @(posedge clk) begin
...
// Timer A: count, overflow, one-shot and toggle logic.
...
// Timer B: count, overflow, one-shot and toggle logic (mirrors Timer A).
...
// Time-of-day clock with alarm and latch.
always @(posedge clk) begin
	if (!res_n) begin
		tod_run     <= 1'b0;
		...
		if (phi2_p) begin
			...
			end else tod_alarm_reg <= 1'b0;
			if (int_reset) icr[2] <= 1'b0;
		end
	end
end

// Serial port: shift register, input/output, ICR[3] flag.
always @(posedge clk) begin
	if (!res_n) begin
		sdr         <= 8'h00;
		sp_out      <= 1'b1;
		...
		if (~cra[6]) begin
			...
			if (sp_pending && ~sp_transmit) begin
				...
				sp_transmit <= 1'b1;
				sp_shiftreg <= sdr;
			end else if (~cnt_out_r && cnt_out) begin
...
// CNT pulse counter and output toggle.
always @(posedge clk) begin
	if (!res_n) begin
		cnt_out_r    <= 1'b1;
		cnt_out      <= 1'b1;
		...
		cnt_in_prev <= cnt_in;
		cnt_out <= cnt_out_r;
		if (cra[6] ? (~cnt_out_r && cnt_out) : (~cnt_in && cnt_in_prev))
			cnt_pulsecnt <= cnt_pulsecnt + 1'b1;
		if (phi2_p) begin
...
// Adjusted ICR with Timer B bug bit substitution.
wire [4:0] icr_adj = {icr[4:2], timer_b_int, icr[0]};

// Interrupt control: mask register, IRQ output, and int_reset strobe.
always @(posedge clk) begin
	reg [7:0] imr_reg;
